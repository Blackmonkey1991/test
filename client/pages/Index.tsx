      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 bg-white rounded-t-3xl">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
          ...
          <div className="space-y-4 md:space-y-6 order-1 lg:order-2">
              ...
              <CardContent>
                <div className="text-center">
                  <div className="text-2xl font-bold text-blue-600 mb-2">
                    {displayOverrides.manualDate && displayOverrides.manualTime
                      ? `${new Date(displayOverrides.manualDate).toLocaleDateString("de-DE", {
                          weekday: "long",
                          year: "numeric",
                          month: "long",
                          day: "numeric",
            ...
            <Card>
              <CardHeader>
                <CardTitle>{t('sidebar.winningClasses')}</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-2 text-sm">
                  ...
                  {WINNING_CLASSES.map((winClass) => (
                    <div key={winClass.class} className="flex justify-between">
                      <span>{winClass.class}</span>
                      <span>{winClass.requirement}</span>
                    </div>
                  ))}
            ...
            {/* Current Winners Statistics */}
            {Object.keys(currentWinners).length > 0 && (
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center space-x-2">
                    <Award className="h-5 w-5" />
                    <span>{t('sidebar.currentWinners')}</span>
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-2 text-sm">
                    ...
                    {Object.entries(currentWinners)
                      .sort(([a], [b]) => parseInt(a) - parseInt(b))
                      .map(([winClass, count]) => (
                        <div key={winClass} className="flex justify-between">
                          <span>{t('sidebar.class')} {winClass}</span>
        ...
        {user && userTickets.length > 0 && (
          <Card className="mt-8">
            <CardHeader>
              <CardTitle>{t('myTickets.title')}</CardTitle>
            </CardHeader>
            <CardContent>
